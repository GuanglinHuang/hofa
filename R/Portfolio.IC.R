...
Portfolio.IC = function(X,r = NULL,rmax = 10,fn_sel = c("ER","GR","IC3","ED"),
  ...
  if(r == 1){
    ...
    f = X%*%A;
    f = f/sqrt(Vm(f,2))
    A = t(lm(X ~ f - 1)$coefficients)
    eps = X - f%*%t(A);
  }else{
    A_pca = eigen(m2)$vector #eigenvalue adjust
    D_pca = eigen(m2)$values
    D_pca[which(D_pca < 0)] <- 0
    X_pca = X%*%A_pca%*%diag(1/sqrt(D_pca))

    ICA = ica::icafast(X_pca,nc = r, fun = "kurt")
    f = ICA$S
    A = A_pca%*%diag(sqrt(D_pca))%*%ICA$M
    eps = X - f%*%t(A)
  }
  ...
  mmP.opt = Portfolio.Moments(w.opt,mm_factor = mmf,mm_eps = mme,A = A)
